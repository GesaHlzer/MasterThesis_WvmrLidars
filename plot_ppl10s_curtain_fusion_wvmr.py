...
ppl10s_raw = xr.open_dataset(os.path.join(os.path.dirname(os.getcwd()),'data','ramanlidar_10s.nc'))
# if using raw add by combine date: date = pl ... .transpose() and change filename
...
def combine_data_on_date(date): #, rsondes, stations, da10):
    ...
    # Lidar as baseline:
    data = pl['wvmr'].copy().T #.to_numpy()  # shape (time, height)
...
for date in dates: 
    ...
    filename = f"PPL10s_wvmr_{hmax}km_{date.strftime('%Y-%m-%d')}.png"
    folderpath = os.path.join(os.path.dirname(os.getcwd()), "plots", "Timeseries", "PPL_10s_wvmr", "CurtainFusion")
