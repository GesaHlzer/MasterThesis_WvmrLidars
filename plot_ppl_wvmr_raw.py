...
ticks = "timewindow"
fig_size = [15, 7] # [15, 5]
start = np.datetime64(date) + np.timedelta64((60*12+00), 'm')
end   = np.datetime64(date) + np.timedelta64((60*20+00), 'm')
# start = np.datetime64(date) + np.timedelta64((60*20+50), 'm') #for 28th
# end   = np.datetime64(date) + np.timedelta64((60*21+40), 'm') #for 28th
# start = np.datetime64(date) + np.timedelta64((60*20+45), 'm') #for 28th
# end   = np.datetime64(date) + np.timedelta64((60*21+50), 'm') #for 28th
...
# ticks = "fullday"
# fig_size = [21,7] # [18, 6]
# date_beg, date_end = datetime(2024, 8, 15), datetime(2024, 8, 15)
...
#     filename = f"PPL{time}_wvmr_{hmax}km_{start.astype('datetime64[h]').astype(str)}_{end.astype('datetime64[h]').astype(str)}_v{version}.png"
#     folderpath = os.path.join(os.path.dirname(os.getcwd()), "plots", "Timeseries", f"PPL_{time}_wvmr", "Raw")
#     savefig(fig, folderpath, filename)