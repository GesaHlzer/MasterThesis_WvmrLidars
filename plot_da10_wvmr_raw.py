...
def plot_da10_wvmr(start, end, vmax, fig_size, ticks):
    ...
    par_cmap = 'Blues'
    param_label = r'wvmr (kg$^{-1}$)'# 'water vapor mixing ratio (g/kg)'
...
folderpath = os.path.join(os.path.dirname(os.getcwd()), "plots", "Timeseries", "DA10_wvmr", "Raw")
savefig(fig, folderpath, filename)

################
# ticks = "fullday"
# fig_size = [21,7] # [18, 6]
# date_beg, date_end = datetime(2024, 8, 15), datetime(2024, 8, 22)
...
#     fig = plot_da10_wvmr(start, end, fig_size, ticks)
#     filename = f"DA10_wvmr_{start.astype('datetime64[h]').astype(str)}_{end.astype('datetime64[h]').astype(str)}_v{version}.png"
#     folderpath = os.path.join(os.path.dirname(os.getcwd()), "plots", "Timeseries", "DA10_wvmr", "Raw")
#     savefig(fig, folderpath, filename)
