...
def read_sl88_stare(date):
    ...
    files = sorted([os.path.join(ncdir, f) 
...
def plot_sl88_stare(date):
    ...
    data_sl88 = read_sl88_stare(date)
    ...
    cbar.ax.tick_params(direction='out', labelsize=16, size=9)
    ...
    ax.set_title(f"SL88: {date.strftime('%Y-%m-%d')}", fontsize=18)
...
for date in dates: 
    filename = f"SL88_stare_{date.strftime('%Y-%m-%d')}.png"
    folderpath = os.path.join(os.path.dirname(os.getcwd()), "plots", "Timeseries", "SL88_stare")
    
    fig = plot_sl88_stare(date)
