# -*- coding: utf-8 -*-
"""
Created on Fri Jan  9 16:33:25 2026
...
def read_sl88_vad(date): # 06.06. - 06.08.
    ...
    file = os.path.join(ncdir, date.strftime('%Y%m%d') + '.nc') 
    
    # Open and concatenate datasets along time
    ds = xr.open_dataset(file, decode_times=False) 
    
    # make time as datetime array & mask out suspiciously large placeholder values
    t = ds["time"].values
    t = np.where(t < 1e12, t, np.nan)  
    t = pd.to_datetime(t, unit="s")
    ds["time"] = ("time", t)
    ds = ds.sel(time=~pd.isnull(ds['time']))
    
    # rename arrays 
    ds = ds.rename({'ucomp': 'u_wind'})
    ds = ds.rename({'vcomp': 'v_wind'})
    
    # Apply intensity threshold
    ds["ff"] = ds["ff"].where(ds["intensity"] >= 1.0045)
    ds["dd"] = ds["dd"].where(ds["intensity"] >= 1.0045)
...
def plot_sl88_vad(date):
    
    # Read data
    data_sl88 = read_sl88_vad(date)
    
    par = data_sl88['ff'].to_numpy() 
    par_cmap = cmap_windspeed()
    
    time = data_sl88['time'].values
    height = data_sl88['height']
    ...
    cbar.ax.tick_params(direction='out', labelsize=18, size=10)
    ...
    ax = wind_barb(ax, data_sl88, time, height)
    ax_inset = wind_barb_legend(ax)
...
for date in dates: 
    ...
    folderpath = os.path.join(os.path.dirname(os.getcwd()), "plots", "Timeseries", "SL88")
    
    fig = plot_sl88_vad(date)
