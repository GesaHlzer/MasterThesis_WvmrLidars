# -*- coding: utf-8 -*-
"""
Created on Tue Jan 13 14:05:55 2026
...
ppl20m   = xr.open_dataset(os.path.join(os.path.dirname(os.getcwd()),'data','ramanlidar_1200s_filtered.nc'))
ppl20m_raw   = xr.open_dataset(os.path.join(os.path.dirname(os.getcwd()),'data', 'ramanlidar_1200s.nc'))

def combine_data_on_date(date): #, rsondes, stations, da10):
    
    # Select timeperiod 
    start = np.datetime64(date - timedelta(minutes=1))
    end   = np.datetime64(date + timedelta(days=1))
    
    pl = ppl20m_raw.copy()
    pl = pl.sel(time=~pl.time.to_index().duplicated())
    pl = pl.sortby("time")
    ...
    # pl = pl.rename({'range': 'height'})
    # pl = pl.rename({'MR': 'wvmr'})
    ...
    # Lidar as baseline:
    data = pl['wvmr'].copy() #.to_numpy()  # shape (time, height)
...
def curtain_plot(date):
    ...
    ax.set_title(f"PPL (20min): {date.date()}", fontsize=fontsize)
...
for date in dates: 
    ...
    filename = f"PPL20m_wvm_rawr_{hmax}m_{date.strftime('%Y-%m-%d')}.png"
    folderpath = os.path.join(os.path.dirname(os.getcwd()), "plots", "Timeseries", "PPL_20m_wvmr", "CurtainFusion")
