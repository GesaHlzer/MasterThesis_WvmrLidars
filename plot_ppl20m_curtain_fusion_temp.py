...
def combine_data_on_date(date): #, rsondes, stations, da10):
    ...
    pl = ppl20m.sel(time=~ppl20m.time.to_index().duplicated())
    pl = pl.sortby("time")
    ...
    for L in rs.launch.values:
        ...
        valid_h = ~np.isnan(rs_i.temp.values)
        data.loc[dict(time=gridtime[time_mask], height=gridheight[valid_h])] = rs_i.temp.values[valid_h]
...
for date in dates:
    ...
    filename = f"PPL20m_temp_{hmax}m_{date.strftime('%Y-%m-%d')}.png"
    folderpath = os.path.join(os.path.dirname(os.getcwd()), "plots", "Timeseries", "PPL_20m_temp", "CurtainFusion")
    savefig(fig, folderpath, filename)
    

# fig = curtain_plot(date)

# filename = f"PPL20m_{date.strftime('%Y-%m-%d')}_{hmax}m.png"
# folderpath = os.path.join(os.path.dirname(os.getcwd()), "plots", "Timeseries", "PPL_20m_temp", "CurtainFusion")
# savefig(fig, folderpath, filename)
